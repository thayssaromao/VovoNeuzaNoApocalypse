...
struct ChoiceThree: View {
    @State private var currentScene: GameScene = ScenesDictionary["SecondZombie"]!
    ...
    @State private var selectedRewardImage: String? = nil
    ...
    var body: some View {
        ...
        if getReward {
            FinalDialog(selectedRewardImage: selectedRewardImage)
        } else if let rewardImage = selectedRewardImage {
                        ...
                        ForEach(currentScene.choices, id: \.destinationId){choice in
                            ChoiceButtonView(title: choice.textButton){
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.6){
                                    if choice.destinationId == "Pão" {
                                        self.selectedRewardImage = "Pão"

                                        self.goToNextScene = true
                                    }
                                    if choice.destinationId == "Cookie" {
                                        self.selectedRewardImage = "Cookie"
